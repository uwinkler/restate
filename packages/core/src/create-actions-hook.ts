...
type ServiceImplementation<S, M extends Message, D extends ServiceImplementationReturn> = (store: RxStore<S, M>) => D
...
export function createActionHooks<S, M extends Message>(provider: RxStoreContext<S, M>) {
  return function createHook<D extends ServiceImplementationReturn>(
    ...
    return function (): D {
      ...
      useEffect(() => {
        const storeSubscription = store.messageBus$.subscribe(async (message) => {
      ...
      return useMemo(
        () =>
          Object.keys(serviceMethods).reduce((prev, name) => {
            const fn = (...args: any) => {
              store.dispatch({ type: serviceName + '/' + name, args: JSON.parse(JSON.stringify(args)) } as any)
            }
            return { ...prev, [name]: fn }
          }, {} as D),
        [store]
...
function isFunction(method: any): method is Function {
