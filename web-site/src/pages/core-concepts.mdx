...
`react-rx-state` follows the three principles of <a href="https://redux.js.org/introduction/three-principles">Redux</a>
...
What Dan Abramov statet for `redux` holds true for react-rx-state` as well:
...
The `react-rx-state` is immutable as well. You will get an exception if you try to bypass `next` and modify
the state directly.
...
In `react-rx-state` you don't write actions yourself. You create actions with the `next` function.
...
In `react-rx-state` the equivalent of reducers are those tiny callback functions you put into `next`:
...
# What distinguishes react-rx-state from redux?
...
Writing asynchronous code is very straight forward in `react-rx-state`.
