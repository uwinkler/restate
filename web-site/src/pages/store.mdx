...
Sometimes it is better to send a message (or action) and let glue-code handle all state changes.
This is particular true with long running operations, async operations or if several parts of the application have to react to a specific event.
