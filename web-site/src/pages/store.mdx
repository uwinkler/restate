...
```code lines=6 src=https://stackblitz.com/edit/restate-create-store
const store = createStore<State>({
  state: {
    name: 'John Snow',
  },
...
```code src=https://stackblitz.com/edit/restate-create-store
store.next(state => {
...
```code variant=ok title="Different ways to use store.next" src=https://stackblitz.com/edit/restate-create-store
...
```code lines=3-11 src=https://stackblitz.com/edit/restate-middleware-logger
...
Sometimes it is convenient to send a message (or action in redux terms) to the glue-code and let glue-code do all the hard work.
...
```code lines=5-18 src=https://stackblitz.com/edit/restate-messagebus
