...
```code lines=6 src=https://stackblitz.com/edit/react-rx-state-create-store
const store = createStore<State>({
  state: {
    name: 'John Snow',
  },
...
```code variant=ok title="Different ways to use store.next" src=https://stackblitz.com/edit/react-rx-state-create-store
...
```code lines=3-11 src=https://stackblitz.com/edit/react-rx-state-middleware-logger
...
```code lines=5-22 src=https://stackblitz.com/edit/react-rx-state-messagebus
...
```code lines=2-12 src=https://stackblitz.com/edit/react-rx-state-glue-meta
...
store.next(s => { s.name = "John Targaryen" })                          // ignored: no meta infos

store.next(s => { s.name = "Sansa Stark" }, { type: 'SEND_TO_SERVER' }) // will be bounced because we send
                                                                        // the next message right away

store.next(s => { s.name = "Bran Stark" }, { type: 'SEND_TO_SERVER' })  // this message goes through
